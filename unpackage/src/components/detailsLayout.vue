	...
	<div style="height: 100%;">
		<yd-layout>
			<yd-navbar slot="navbar" :title="title">
				...
				<yd-icon name="touchengkongyun" v-if="$route.path == '/offerZoneDetail'"  slot="right" size='.5rem' @click.native="shareToH5(0)" custom></yd-icon>
			</yd-navbar>
			<slot></slot>
			<hr slot="tabbar">
			<yd-flexbox slot="tabbar">
				...
				<yd-flexbox-item v-if="moreFun.length > 0" @click.native="moreShow = true">
					<div class="func-button-more">
						...
						<i class="iconfont icon-gengduo-hengxiang" style="font-size:32px"></i>
					</div>
				</yd-flexbox-item>
			</yd-flexbox>
		</yd-layout>
	...
	export default {
		...
		created() {
			...
			for(let i = 0; i < this.funcs.length; i++) {
				if(this.funcs[i].limit == false) {
					this.funcs.splice(i, 1);
					i--;
				}
			};
			for(let i = 0; i < this.moreFun.length; i++) {
				if(this.moreFun[i].limit == false) {
					this.moreFun.splice(i, 1);
					i--;
				}
			}
		}
