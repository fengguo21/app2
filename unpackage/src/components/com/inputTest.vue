    ...
    <div>
        <div class="wrapBox">
            ...
            <div style="display:inline-block;width:60%;position:relative;" v-if="this.type=='time'">
                <yd-datetime style="height: 32px;line-height: 32px;text-align: left ; font-size:0.3rem; color:#777777;" placeholder="请选择时间" :start-year="2015" :init-emit="false" v-model="val"></yd-datetime>
            </div>
            <!-- type="address"地址选择 -->
    ...
    export default {
        ...
        watch: {
            val: function(newv, oldv) {
                console.log("阿啥都看见巴萨科技电脑还是",this.val)
                if(this.type == 'time' || this.type == 'address') {
                    this.$emit('update:param', this.val);
                }
            },
            param: function(newv, oldv) {
                console.log("阿888888888888888888888888脑还是",this.val)
